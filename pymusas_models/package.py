...
This code is used to package the spaCy models.

The main differences between this and the original spaCy code:

1. Custom README generator that adds:
    * The model size.
2. Does not require the `lang` from the meta file to be the first word of the
model name. This is to overcome the problem when the model name starts with the
...
from spacy import about, util
from spacy.cli._util import SDIST_SUFFIX, WHEEL_SUFFIX, Arg, Opt, app, string_to_list
...
def package_cli(
    ...
    code_paths_strings: str = Opt("", "--code", "-c", help="Comma-separated paths to Python file with additional code (registered functions) to be included in the package"),
    ...
    code_paths = [Path(p.strip()) for p in string_to_list(code_paths_strings, False)]
    package(
        input_dir,
        output_dir,
        meta_path=meta_path,
